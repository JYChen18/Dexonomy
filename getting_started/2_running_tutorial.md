...
```bash
python -m dexonomy.main op=tmplhand=allegro
```
...
```bash
python -m dexonomy.main op=init hand=allegro exp_name=first_try template_name=fingertip_small 'gpu_list=[0]' op.object.cfg_num=20
```
...
```bash
python -m dexonomy.main op=grasp hand=allegro exp_name=first_try
```
...
#### Debugging Options
```bash
python -m dexonomy.main op=grasp hand=allegro exp_name=first_try skip=False debug_name=core_bottle_523cddb320608c09a37f3fc191551700 debug_render=True hydra.verbose=true 
```
...
```bash
python -m dexonomy.main op=test hand=allegro exp_name=first_try
```
...
```bash
python -m dexonomy.script hand=allegro exp_name=first_try 'template_name=[fingertip_small,fingertip_mid,fingertip_large]' 'gpu_list=[5,6,7]' +init.object.cfg_num=20 '+grasp.grasp.qp_filter.miu_coef=[0.5, 0.02]'
```
...
```bash
python -m dexonomy.main op=vusd hand=allegro exp_name=first_try op.max_num=20 op.data_type=init op.check_success=False
```
- `op.max_num`: Limit number of data to visualize.
- `op.data_type`: Specifies which data directory to visualize. 
...
- `op.check_success`: Filters the data to visualize
...
```bash
python -m dexonomy.main op=v3d hand=allegro exp_name=first_try op.data_type=init op.object.contact=True op.hand.contact=True 
```
...
```bash
python -m dexonomy.main op=stat hand=allegro exp_name=first_try 
```
