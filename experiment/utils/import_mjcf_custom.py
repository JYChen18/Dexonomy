...
def parse_mjcf(
    ...
    if compiler is not None:
        ...
        euler_seq = [
            "xyz".index(c) for c in compiler.attrib.get("eulerseq", "xyz").lower()
        ]
    ...
    def parse_default(node, parent):
        ...
        if class_name not in class_defaults:
            class_defaults[class_name] = deepcopy(
                class_defaults[class_parent[class_name]]
            )
        for child in node:
            if child.tag == "default":
                parse_default(child, node.get("class"))
            else:
                if child.tag not in class_defaults[class_name].keys():
                    class_defaults[class_name][child.tag] = child.attrib
                else:
                    class_defaults[class_name][child.tag].update(child.attrib)
    ...
    def parse_shapes(
        ...
        for geo_count, geom in enumerate(geoms):
            ...
            geom_name = geom_attrib.get(
                "name",
                f"{body_name}_geom_{geo_count}{'_visual' if just_visual else ''}",
            )
            ...
            if geom_type == "sphere":
                ...
                if "mesh" in geom_defaults:
                    mesh_scale = parse_vec(
                        geom_defaults["mesh"],
                        "scale",
                        mesh_assets[geom_attrib["mesh"]]["scale"],
                    )
                else:
                ...
                if hasattr(m, "geometry"):
                    ...
                    for m_geom in m.geometry.values():
                        m_vertices = (
                            np.array(m_geom.vertices, dtype=np.float32) * scaling
                        )
                ...
                if geom_type == "cylinder":
                    tm = trimesh.creation.cylinder(
                        radius=geom_radius, height=2 * geom_height
                    )
                    s = builder.add_shape_mesh(
                        body=link,
                        pos=geom_pos,
                        rot=geom_rot,
                        mesh=Mesh(
                            np.array(tm.vertices, dtype=np.float32),
                            np.array(tm.faces.flatten(), dtype=np.int32),
                        ),
                        density=density,
                        is_visible=visible,
                        has_ground_collision=not just_visual,
                ...
                if verbose:
                    print(
                        f"MJCF parsing shape {geom_name} issue: geom type {geom_type} is unsupported"
                    )
        return shapes

    def parse_body(
        body, parent, incoming_defaults: dict, childclass: str | None = None
    ):
            ...
            for _i, joint in enumerate(joints):
                ...
                if "class" in joint.attrib:
                    ...
                    if joint_class in class_defaults:
                        joint_defaults = merge_attrib(
                            joint_defaults, class_defaults[joint_class]
                        )
                ...
                joint_pos.append(
                    parse_vec(joint_attrib, "pos", (0.0, 0.0, 0.0)) * scale
                )
                joint_range = parse_vec(
                    joint_attrib, "range", (joint_limit_lower, joint_limit_upper)
                )
                joint_armature.append(
                    parse_float(joint_attrib, "armature", armature) * armature_scale
                )
        ...
        if (
            len(freejoint_tags) > 0 or floating is not None or base_joint is not None
        ) and parent == -1:
            ...
            if base_joint is not None:
                ...
                base_child_xform = wp.transform(
                    (0.0, 0.0, 0.0), wp.quat_inverse(_xform.q)
                )
            ...
            elif floating or len(freejoint_tags) > 0:
                ...
                builder.add_joint_fixed(
                    -1, link, parent_xform=_xform, name="fixed_base"
                )
        ...
        parse_shapes(
            defaults, body_name, link, colliders, density, visible=show_colliders
        )
    ...
    world_defaults = merge_attrib(
        class_defaults["__all__"], class_defaults.get(world_class, {})
    )
