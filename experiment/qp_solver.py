...
class ContactQP:
    ...
    def gradient(self,
        ...
        wrench_total = np.sum(contact_wrenches, axis=0) + gravity # (6, )
        wrench_error = np.linalg.norm(wrench_total)
        
        # ignore normal's derivate for now
        grad = np.cross(contact_wrenches[:, :3], wrench_total[3:][None, ...], axis=-1) # (n, 3)
        # project grad to tangent space
        grad = grad - np.sum(grad * normal, axis=-1, keepdims=True) * normal
        
        return grad, contact_wrenches, wrench_error
